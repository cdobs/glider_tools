...
def calc_o2(cph, tmp, C, M, N, cc=(0, 1), S=0.0,
    ...
    * if the Salinity calibration parameter within the optode is set to
    ...
    airsaturation = cc[0] + airsaturation*cc[1]
...
def do2_SVU(calphase, temp, csv, conc_coef=np.array([0.0, 1.0]), salt=0):
    ...
    * if the Salinity calibration parameter within the optode is set to
    ...
    conc_coef : 'secondary' calibration coefficients: an array of offset and
        ...
        conc_coef[0, 1] = slope
		default is (0, 1)
    salt : preset salinity parameter on the optode (default is 0)

    Returns
    -------
	DO : Dissolved oxygen concentration, [micro-moles/kg]
		OOI Data Product DOCONCS_L2a corrected for salinity and pressure
...
def SVU(cal, tmp, calsv, conc_coef=[0, 1], SP=0):
    """Implementation of Aanderaa's version of the
    Sterm-Volmer-Uchida Equation with partial salinity correction
    if it is preset in the optode (`SP`)
    
    parameters
    ----------
    cal: calphase: float or array of floats. [deg]
        The raw measured output Calibrated Phase from the optode
    tmp: optode temperature: float or array of float [dec C]
        Temperature ouput from the optode's thermistor. This is
        required to be optode temperature as opposed to collocated
        CTD temperature because the optode calibration parameters
        `calsv` were calculated using the optode's thermistor.
        CTD temperature can be used later in the salinity/pressure
        correction.  Should be the same size as `cal`.
     calsv: SVU calibration parameters: at least 7 element array-like
        The SVUFoilCoef coefficients from the optode's calibration
        certificate.  This can be either a 1D 7 element array-like, 
        or an 2D len(`cal`)x7 array-like with the appropriate
        coefficients for each `cal` value.
    
    returns
    -------
    DO: Molar Dissolved Oxygen concentration. [uM]
        Not yet compensated for CTD salinity and pressure
    """
    # This one is more up-to-date than the ion-functions one above
    
    # make sure that calsv is a 2D array for proper indexing
    calsv = np.atleast_2d(calsv)
    conc_coef = np.atleast_2d(conc_coef)
    
    cal = np.atleast_1d(cal)
    tmp = np.atleast_1d(tmp)
    
    # SVU algorithm
    Ksv = calsv[:, 0] + calsv[:, 1]*tmp + calsv[:, 2]*(tmp**2)
    P0 = calsv[:, 3] + calsv[:, 4]*tmp
    Pc = calsv[:, 5] + calsv[:, 6]*cal
    DOc = ((P0/Pc) - 1) / Ksv
    
    # salinity correction using the preset salinity within the optode
    # and the optode temperature
    S0 = 0
    ts = np.log((298.15-tmp)/(273.15+tmp))
    B0 = -6.24097e-3
    B1 = -6.93498e-3
    B2 = -6.90358e-3
    B3 = -4.29155e-3
    C0 = -3.11680e-7
    Bts = B0 + B1*ts + B2*ts**2 + B3*ts**3
    DOc = np.exp((SP-S0)*Bts + C0*(SP**2-S0**2)) * DOc
    
    DOc = conc_coef[:, 0] + conc_coef[:, 1] * DOc
    return DOc